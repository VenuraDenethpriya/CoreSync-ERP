...
const cloudinaryAudioURL = import.meta.env.VITE_CLOUDINARY_AUDIO_URL;
...
export const uploadImageToCloudinary = async (file) => {
  ...
  formData.append("upload_preset", "rims_preset"); 
...
export const uploadAudioToCloudinary = async (file) => {
  console.log("=== Cloudinary Upload Debug ===");
  console.log("Cloudinary URL:", cloudinaryAudioURL);
  console.log("File details:", {
    name: file.name,
    size: file.size,
    type: file.type
  ...
  const formData = new FormData();
  formData.append("file", file);
  formData.append("upload_preset", "rims_preset"); // Must allow unsigned upload

  try {
    const response = await fetch(cloudinaryAudioURL, {
      method: "POST",
      body: formData
    });
