...
import { TalentTable } from "./Components/TableCard.page";
...
const TalentPool = () => {
  ...
  const breadcrumbs = [
    { name: "Dashboard", link: "/" },
    { name: "Talent Pool", link: "/hr/talent-pool" },
  ];
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      doc.text("Talent Pool List", 14, 15);
      doc.save("Talent Pool List.pdf");
      ...
      await createAuditLog(token, {
        ...
        status_code: 200,
        user: user.id,
        description: `${user.firstName} ${user.lastName} downloaded the talent pool list as a PDF file.`,
      });
      ...
      {isMobile ? (
        <div className="flex flex-col w-full">
          <div className="sticky top-0 z-30 flex items-center justify-between px-4 py-4 bg-white border-b shadow-sm w-full">
            <h1 className="font-bold text-xl text-gray-900">Talent Pool</h1>
            <Button
          ...
          <div className="w-full max-w-[100vw] overflow-x-auto overflow-y-hidden">
            <div className="min-w-[700px]  py-2">
              <TalentTable setExportData={setExportData} />
            </div>
          </div>
        </div>
      ) : (
        <>
          ...
          <div className="mx-3 sm:mx-16">
            <TalentTable className="" setExportData={setExportData} />
          </div>
        </>
      )}
    </div>
