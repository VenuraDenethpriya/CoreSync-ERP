...
export function TalentTable({ setExportData }) {
  ...
  const columns = [
      ...
      cell: ({ row }) => {
        ...
        return (
          <div
            ...
            onClick={() => navigate(`/hr/talent-pool/${id}`)}
          >
            {name}
          </div>
        )
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const { dateOfApplication, id } = row.original
        return (
          <div
            className="capitalize cursor-pointer"
            onClick={() => navigate(`/hr/talent-pool/${id}`)}
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const { evaluationScore, id } = row.original
        return (
          <div
            onClick={() => navigate(`/hr/talent-pool/${id}`)}
            className="capitalize pl-5" >{row.getValue("evaluationScore")}</div>
        )
      },
    },
    {
