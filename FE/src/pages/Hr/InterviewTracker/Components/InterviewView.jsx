...
function InterviewView() {
    ...
    const breadcrumbs = [
        { name: "Dashboard", link: "/" },
        { name: "Interviews-tracker", link: "/hr/interviews-tracker" },
        { name: " ", link: `/hr/interviews-tracker/${memberId}` },
    ];
    ...
    return (
        <>
            ...
            <div className="sm:p-8 p-4 space-y-6 bg-gray-50 min-h-screen">
                ...
                <div className="bg-white rounded-xl shadow-md p-6 border border-gray-200">
                    <h3 className="text-xl font-bold mb-5 text-gray-700">Applicant Details</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8">
                        <DetailRow label="Applicant Name" value={customerData.first_name} />
                        <DetailRow label="Phone No ." value={customerData.phone_no} />
                        <DetailRow label="Email" value={customerData.email} />
                        <DetailRow label="Expected Salary" value={customerData.expected_salary} />
                        <DetailRow label="Current Employer" value={customerData.position_applied} />
                        <DetailRow label="Current Position" value={customerData.date_of_application ? new Date(customerData.date_of_application).toLocaleDateString() : "N/A"} />
                        <DetailRow label="Resume" value={customerData.resume} />
                        <DetailRow label="Join Date" value={customerData.resume} />
                    </div>
                </div>

                <div className="bg-white rounded-xl shadow-md p-6 border border-gray-200">
                    <h3 className="text-xl font-bold mb-5 text-gray-700">Interview Details</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8">
                        <DetailRow label="Date Applied" value={customerData.date_of_application ? new Date(customerData.date_of_application).toLocaleDateString() : "N/A"} />
                        <DetailRow label="Date of Interview" value={customerData.interview_date ? new Date(customerData.interview_date).toLocaleDateString() : "N/A"} />
                        <DetailRow label="Interviewer" value={customerData.interviewer} />
                        <DetailRow label="Interview Score" value={customerData.expected_salary} />
                        <DetailRow label="Notes" value={customerData.position_applied} />
                    </div>
                </div>

                <div className="bg-white rounded-xl shadow-md p-6 border border-gray-200">
                    <h3 className="text-xl font-bold mb-5 text-gray-700">Position Details</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8">
                        <DetailRow label="Position Title" value={customerData.date_of_application ? new Date(customerData.date_of_application).toLocaleDateString() : "N/A"} />
                        <DetailRow label="Department" value={customerData.interview_date ? new Date(customerData.interview_date).toLocaleDateString() : "N/A"} />
                        <DetailRow label="Expected Salaray" value={customerData.interviewer} />
                        <DetailRow label="Experience Required" value={customerData.expected_salary} />
                    </div>
                </div>
