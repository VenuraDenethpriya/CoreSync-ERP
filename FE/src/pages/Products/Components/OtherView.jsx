...
import OtherDetailsTemplate from "./OtherPdf";
...
function OtherView() {
  ...
  useEffect(() => {
    ...
    useEffect(() => {
        ...
        fetchProductData();
    }, [getToken, productId, isLoaded]);

    const handleDelete = async () => {
        try {
            const token = await getToken();
            await deleteProduct(token, productId);
            toast.error("Other product deleted successfully!", { position: "bottom-right" });
            navigate("/products");
        } catch (error) {
            ...
            <div className="sm:p-8 p-4 space-y-6 bg-gray-50 min-h-screen">
                ...
                <div className="bg-white rounded-xl shadow-sm p-6 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                    <h1 className="sm:text-4xl text-2xl font-extrabold text-gray-800">{productData.name}</h1>
                    {
                        ...
                        <EditOther productData={productData} onUpdate={handleProductUpdate} />
                      
                            <DeleteAlert handleDelete={handleDelete} productData={productData} />
                       
                        <PDFDownloadLink
                            document={<OtherDetailsTemplate productData={productData} />}
                            fileName={`Other-Product-Report-${productData.name}.pdf`}
                    ...
                    <div className="bg-white rounded-xl shadow-md p-6 border border-gray-200">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8">
                            <div className="space-y-3">
                                <DetailRow label="Voltage(V)" value={productData.voltage} />
                                <DetailRow label="Brand Name" value={productData.Specifications} />
                            </div>
                            <div className="space-y-3">
                            </div>
                        </div>
